...
class ProcessingConstants:
    ...
    CSV_COLUMNS = ['ID', 'DATE', 'ELEMENT', 'VALUE', 'MFLAG', 'QFLAG', 'SFLAG', 'OBSTIME']
...
def calculate_progress_percentage(current: int, total: int, base: int = 10, max_progress: int = 90) -> int:
    if total <= 0:
        return base
...
def filter_target_years(dag_run_conf: Optional[Dict], default_years: List[str] = None) -> List[str]:
...
def log_chunk_progress(chunk_number: int, chunk_size: int, progress_percent: int):
    print(f"📊 Chunk {chunk_number}: {chunk_size:,} records | Progress: {progress_percent}%")


def log_error_details(filename: str, error: Exception, processing_state: ProcessingState):
    print(f"❌ Failed to process {filename} in phase '{processing_state.phase}': {str(error)}")
    print(f"🔍 Error details: {type(error).__name__} at {processing_state.progress_percent}% completion") 