...
def consolidate_results(results: List[Dict[str, Any]]) -> str:
    successful_files = [
        r for r in results if r is not None and r.get("status") != "failed"
    ]
    ...
    if failed_count > 0:
        ...
        for failed_file in failed_files:
            print(
                f"   • {failed_file['file_name']}: {failed_file.get('error_type', 'Unknown')} - {failed_file.get('error', 'No details')}"
            )
...
def _build_consolidated_report(
    ...
    return {
        ...
        "processing_summary": {
            ...
            "success_rate": (
                round(len(successful_files) / len(results) * 100, 1) if results else 0
            ),
        },
...
def _calculate_aggregate_statistics(
    ...
    total_unique_stations = len(
        set().union(
            *[set(range(r.get("unique_stations", 0))) for r in successful_files]
        )
    )
    ...
    average_processing_time = round(
        sum(r.get("processing_seconds", 0) for r in successful_files)
    ...
    return [
        {
            ...
            "progress_when_failed": f.get("processing_state", {}).get(
                "progress_percent", 0
            ),
        }
...
def _log_final_metrics(consolidated: Dict[str, Any]):
    ...
    print(
        f"📈 Total records processed: {consolidated['aggregate_statistics']['total_records_across_all_files']:,}"
    )
    print(
        f"🗓️ Date range: {consolidated['aggregate_statistics']['date_range_overall']['earliest']} to {consolidated['aggregate_statistics']['date_range_overall']['latest']}"
    )
    print(
        f"⏱️ Average processing time: {consolidated['aggregate_statistics']['average_processing_time_per_file']}s per file"
    )
