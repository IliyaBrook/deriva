...
def process_file(filename: str) -> Dict[str, Any]:
    ...
    try:
        ...
        print(
            f"⚙️ [{processing_state.phase}] Processing {filename} with pandas chunking..."
        )
        ...
        max_date = None
        measurement_counts = {
            element: 0 for element in ProcessingConstants.MEASUREMENT_TYPES
        }
        tmax_sum = 0.0
        tmax_count = 0
        tmin_sum = 0.0
        ...
        with gzip.open(io.BytesIO(response.content), "rt") as f:
            ...
            for chunk_df in pd.read_csv(
                ...
                if chunk_number == 1:
                    ...
                    total_chunks = max(
                        1, estimated_rows // ProcessingConstants.CHUNK_SIZE
                    )
                    processing_state.update_progress(chunk_number, total_chunks)

                log_chunk_progress(
                    chunk_number, len(chunk_df), processing_state.progress_percent
                )
                ...
                if len(unique_stations) < ProcessingConstants.MAX_UNIQUE_STATIONS:
                    unique_stations.update(
                        chunk_df["ID"].unique()[
                            : ProcessingConstants.MAX_STATIONS_PER_CHUNK
                        ]
                    )
                ...
                try:
                    chunk_df["DATE"] = pd.to_datetime(
                        chunk_df["DATE"], format="%Y%m%d", errors="coerce"
                    )
                ...
                for element in ProcessingConstants.MEASUREMENT_TYPES:
                    measurement_counts[element] += (
                        chunk_df["ELEMENT"] == element
                    ).sum()

                # Calculate temperature statistics
                delta_tmax_sum, delta_tmax_count = safe_execute(
                    calculate_element_stats, chunk_df, "TMAX", default_value=(0, 0)
                )
                tmax_sum += delta_tmax_sum
                tmax_count += delta_tmax_count

                delta_tmin_sum, delta_tmin_count = safe_execute(
                    calculate_element_stats, chunk_df, "TMIN", default_value=(0, 0)
                )
        ...
        print(
            f"✅ [{processing_state.phase}] Finished processing {filename}: {total_records:,} total records in {chunk_number} chunks"
        )
